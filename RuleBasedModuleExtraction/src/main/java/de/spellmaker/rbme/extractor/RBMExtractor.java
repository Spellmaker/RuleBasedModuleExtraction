...
public class RBMExtractor {
	...
	private int doubleDefs = 0;
	
	private Integer[] defDeclPos;
	private Integer[] ruleAxioms;
	
	public RBMExtractor(boolean doDefinitions){
			...
			for(Integer cRule : matchRules){
				...
				if(--ruleCounter[cRule] <= 0){
					...
					if(head == null){
						...
						if(definedSymbol != null && !knownNotBottom[definedSymbol]){
	...
	public int getDoubleDefs(){
		return doubleDefs;
	}
	
	private boolean addQueue(Integer o){
		if(definitions[o] != null && definitions[o] >= 0){
